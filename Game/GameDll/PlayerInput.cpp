...
void CPlayerInput::OnAction( const ActionId& actionId, int activationMode, float value )
{
	...
	if (actionId == "attack1")
	{
		...
		if (activationMode == EActionActivationMode::eAAM_OnPress)
		{
			...
			// Spawn Our Proximity Mine With The Specified Parameters
			SEntitySpawnParams SP;

			//The Entity Should Cast Shadows... Although We Could Make This A Lua Script Property And Read It In On The "PostInit" Function Of Our ProximityMine 
			SP.nFlags = EEntityFlags::ENTITY_FLAG_CALC_PHYSICS | EEntityFlags::ENTITY_FLAG_CASTSHADOW;

			// The Entity Should Use Our Proximity Mine IGameObjectExtension
			SP.pClass = gEnv->pEntitySystem->GetClassRegistry()->FindClass("ProximityMine");
			SP.qRotation = IDENTITY;
			SP.sName = "ProximityMine1";
			...
			if (pProximityMine)
			{
				...
				IGameObject *pGameObject = gEnv->pGameFramework->GetGameObject(pProximityMine->GetId());
				...
				// Throw The Mine Away From Us By Using Physics Impulses
				IPhysicalEntity *pPhysEnt = pProximityMine->GetPhysics();
				if (pPhysEnt)
				{
					pe_action_impulse Impulse;

					// Apply The Impulse This Frame
					Impulse.iApplyTime = 0;

					// The Mine Should Be Trown Away From Us In Our Looking Direction
					Vec3 Dir = m_pPlayer->GetEntity()->GetForwardDir().normalized();

					// Throw At A Rate Of 50 Meters Per Second
					Dir.SetLength(50);
					Impulse.impulse = Dir;
